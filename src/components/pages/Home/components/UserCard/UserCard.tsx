...
const UserCard = ({ user }: { user: IUser }): ReactElement => {
  ...
  return <div className={[style.container].join(' ')}>
    <h3>{user.name}</h3>
    <div><Icon iconName="mail" className={style.inlineIcon} wrapperStyle="inline" />&nbsp;{user.email}</div>
    <div><Icon iconName="call" className={style.inlineIcon} wrapperStyle="inline" />&nbsp;{user.phone}</div>
